...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbors: List[(Block, Move)] = List((this.left, Left), (this.right, Right), (this.down, Down), (this.up, Up))

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors filter (block => terrain(block._1.b1) && terrain(block._1.b2))
